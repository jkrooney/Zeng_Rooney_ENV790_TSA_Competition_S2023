...
#couldn't get 8.1 to run

model8 <- nnetar(clean_ts_train,p=1,P=1)
#model8.1 <- nnetar(clean_ts_train,p=1,P=0,xreg=fourier(clean_ts_train, K=c(2,4), h=424))

model8_forecast <- forecast(model8, h=424) 
#model8.1_forecast <- forecast(model8, h=424,xreg=fourier(clean_ts_train, K=c(2,4),h=424))
...
#Plot model + observed data
autoplot(clean_ts_test) +
  autolayer(model8_forecast, series="Neural Network",PI=FALSE)+
  ylab("Active Power") 
